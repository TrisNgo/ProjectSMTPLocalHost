...
namespace ProjectSMTPLocalHost
{
    public partial class FrmSmtpLocal : Form
    {
        ...
        private void authenCheck(string input)
        {
            ...
            if (comboBox1.SelectedIndex == 1 && count == 13) //if the server is using is mdaemon
            {
                stop = true;
                return;
            }
            else if (comboBox1.SelectedIndex == 2 && count == 10) //if the server is using is hmail
            {
                stop = true;
                return;
            }
            else if (comboBox1.SelectedIndex == 3 && count == 7) ////if the server is using is self-build server
            {
                stop = true;
                return;
            }
        ...
        private void addTextToBox1(string input)
        {
            string text = "S: " + input + "\n";
            txtBoxGetSer.Text += text;
            if ((comboBox1.SelectedIndex == 3 && input.StartsWith("221")) || (comboBox1.SelectedIndex == 2 && input.StartsWith("354")) || (comboBox1.SelectedIndex == 1 && input.StartsWith("221")))
            {
        ...
        private void btnAttach_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                txtBoxAttachments.Text += fileName[index] + "\n";
                index++;
            }
            catch
            {
        ...
        private void bttnClrAttach_Click(object sender, EventArgs e)
        {
            ...
            txtBoxAttachments.Text = "";
        }
        private void btnSend_Click(object sender, EventArgs e)
        {
            if (comboBox1.SelectedIndex == 0)
            {
                MessageBox.Show("Vui lòng chọn chính xác server bạn đang sử dụng, chọn sai chương trình sẽ bị lỗi!", "Thông báo");
            }
            else
            {
                int port;
                ...
                else
                {
                    ...
                    if (filePath == null || filePath.Length == 0)
                        ...
                        hasAttachment = true;
                    if (!hasAttachment) MessageBox.Show("Self-build server không thể nhận attachment, chương trình sẽ gửi mail đi mà không có attachment!", "Thông báo");
                    Array.Clear(filePath, 0, filePath.Length);
                    Array.Clear(fileName, 0, fileName.Length);
                    txtBoxAttachments.Text = "";*/
                    port = 1024;
                ...
                {
                    MessageBox.Show("Vui lòng kiểm tra server đã hoạt động hay chưa và thử lại");
                    return;
                }
        ...
        private void authenMess()
        {
            ...
            {
                MessageBox.Show("Có lỗi xảy ra, vui lòng kiểm tra lại server có hoạt động không");
            }
        }
